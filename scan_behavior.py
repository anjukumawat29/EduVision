...
def classify_behavior(detected, frame):
    ...
    if "cell phone" in detected:
        return "using phone", "phone in hand"

    if "person" not in detected:
        return "distracted", "no person in frame"
    ...
    if any(c in detected for c in {"book", "laptop", "tv", "monitor"}):
        return "attentive", "studying"

    return "attentive", "facing forward"
...
def draw_overlay(frame, results, behavior, reason, elapsed, total_dur):
    ...
    # Top HUD
    cv2.rectangle(frame, (0, 0), (frame.shape[1], 62), (15, 23, 42), -1)
...
print("[behavior] Warming up camera...")
for _ in range(40):
    cam.read()
...
while True:
    ...
    cv2.imshow("EduVision -- Behavior Monitor", frame)
